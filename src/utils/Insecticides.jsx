  ...
  const fetchProducts = async () => {
    try {
      ...
      if (data.success && Array.isArray(data.data)) {
        const insecticides = data.data.filter(
          (item) =>
            item.category &&
            item.category.trim().toLowerCase().startsWith("insecticide"),
        );

        setProducts(insecticides);
        setFilteredProducts(insecticides);

        if (insecticides.length === 0) {
          toast.info("No insecticide found.");
        }
      } else {
        setProducts([]);
        setFilteredProducts([]);
    ...
    <Container>
      ...
      <div className="productsGrid">
        {Array.isArray(filteredProducts) && filteredProducts.length > 0 ? (
          ...
          <p className="paragraph">No Insecticide found</p>
        )}
      </div>
      {detail && (
